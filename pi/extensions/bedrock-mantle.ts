 ...
 * AWS credentials from process.env, and register a `bedrock-mantle` provider
 * whose baseUrl points at the proxy.
 * AWS credentials are managed outside pi (e.g. aws sso login / aws configure
 * export-credentials); this extension only reads them.
 */
...
function signV4(opts: {
  ...
  if (opts.creds.sessionToken)
    headers["x-amz-security-token"] = opts.creds.sessionToken;
...
async function handle(req: IncomingMessage, res: ServerResponse) {
    ...
    res.end(
      JSON.stringify({
        error: {
          ...
          message:
            "No AWS credentials in process.env (AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY). Authenticate outside Pi, then restart Pi.",
        },
      }),
    );
    return;
  ...
  try {
    upstream = await fetch(
      `https://${host}${rawPath}${rawQuery ? `?${rawQuery}` : ""}`,
      {
        method: req.method || "POST",
        headers: outHeaders,
    ...
    res.end(
      JSON.stringify({
        error: {
          type: "api_error",
          message: `bedrock-mantle proxy upstream failure: ${String(error)}`,
        },
      }),
    );
    return;
...
async function startProxy(): Promise<number> {
  ...
  if (!addr || typeof addr !== "object")
    throw new Error("bedrock-mantle proxy failed to bind");
  return addr.port;
}
    ...
    handler: async (_args, ctx) => {
      ...
      ctx.ui.notify(
        `proxy ${base} -> https://${upstreamHost(region)} | creds: ${
          creds
            ? `${creds.accessKeyId.slice(0, 8)}… (session token: ${creds.sessionToken ? "yes" : "no"})`
            : "MISSING"
        }`,
        creds ? "info" : "warning",
      );
